...
// --- FUNGSI UNTUK KESALAHAN (localStorage) ---
function parseReportText(text) {
    ...
    return {
        ...
        full_text: text // Menyimpan teks laporan lengkap
    };
}
...
function updateDashboard() {
    ...
    let filteredErrors = errors.filter(e => {
        const errorDate = new Date(e.tanggal);
        const dateMatch = (!fromDate || errorDate >= fromDate) && (!toDate || errorDate <= toDate);
    ...
    filteredErrors.sort((a,b)=>b.id-a.id).forEach(err=>{
        const row = `
            <tr>
                <td>${err.id}</td>
                <td>${new Date(err.tanggal).toLocaleString("id-ID")}</td>
                <td>${err.staff}</td>
                <td>Staff</td>
                <td>${err.perihal}</td>
                <td>
...
// --- FUNGSI UNTUK DATA STAFF (localStorage) ---
